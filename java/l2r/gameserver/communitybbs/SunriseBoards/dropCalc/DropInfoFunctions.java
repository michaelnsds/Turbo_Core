...
public class DropInfoFunctions
{
	...
	public static void showNpcDropList(L2PcInstance activeChar, String dropType, int npcId, int page)
	{
		...
		replyMSG.append(")<br>");
		replyMSG.append(dropType.equals("CORPSE") ? "<button value=\"Show Drops\" action=\"bypass -h _bbssearchNpcDropList_DEATH_" + npcId + "_" + 1 + "\" width=280 height=20 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_CT1.Button_DF\"></td>" : "<td width=120><button value=\"Show Spoils\" action=\"bypass -h _bbssearchNpcDropList_CORPSE_" + npcId + "_" + 1 + "\" width=280 height=20 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_CT1.Button_DF\">");
		...
		if (npcData.getDropData() != null)
		{
			for (L2DropCategory cat : npcData.getDropData())
			{
				...
				if (dropType.equals("CORPSE") && cat.isSweep())
				{
					for (L2DropData drop : cat.getAllDrops())
					{
						if (DropInfoHandler.HERBS.contains(drop.getId()))
						{
							continue;
						}
						...
						replyMSG.append("<button action=\"bypass -h _bbssearchdropMonstersByItem_" + item.getId() + "_" + 1 + "\" width=32 height=32 back=\"L2UI_CT1.ItemWindow_DF_Frame_Down\" fore=\"L2UI_CT1.ItemWindow_DF_Frame\">");
						replyMSG.append("</td>");
						replyMSG.append("</tr>");
						replyMSG.append("</table>");
						replyMSG.append("</td>");
						...
						replyMSG.append("<br1><font color=\"5882FA\">" + Util.formatDouble(drop.getChance() / 10000, "#.##") + "% to </font>" + "" + (drop.isQuestDrop() ? "Quest" : (cat.isSweep() ? "Spoil" : "Drop")));
						replyMSG.append("</td>");
						replyMSG.append("</tr>");
						replyMSG.append("</table>");
				...
				else if (dropType.equals("DEATH") && !cat.isSweep())
				{
					for (L2DropData drop : cat.getAllDrops())
					{
						if (myPage != page)
						...
						final L2Item item = ItemData.getInstance().getTemplate(drop.getId());
						replyMSG.append("<br><center><img src=\"l2ui.squaregray\" width=\"270\" height=\"2\"></center>");
						replyMSG.append("<center><table border=0 cellpadding=0 cellspacing=0 width=\"270\" height=\"40\" bgcolor=\"2E2E2E\">");
						replyMSG.append("<tr>");
